...
runners.run = function(promiseImplementation, Defer, runLoops, cb) {
...
runners.asyncNewPromise = function(i, perf) {
  ...
  setTimeout(function(){
    ...
    promise = app.promise(Prom, defer, runners.asyncResolve);
    ...
    if (loops === i + 1){
      Prom.all(promises)
        .then(runners.finish.bind(null, perf, runners.loopStart), console.log)
