...
runners.run = function(promiseImplementation, runLoops, cb, optAsyncResolve) {
  ...
  if (false === promiseImplementation) {
    runners.loopStartAsync(optAsyncResolve);
  } else {
    runners.loopStart(optAsyncResolve);
  }
};

runners.loopStart = function(optAsyncResolve) {
  ...
  for (var i = 0; i < loops; i++) {
    runners.asyncNewPromise(i, perf, optAsyncResolve);
  }
  perf.log('after for');

};

runners.loopStartAsync = function(optAsyncResolve) {
  ...
  for (var i = 0; i < loops; i++) {
    functions.push(app.vanilla.bind(null, perf, optAsyncResolve));
  }
...
runners.asyncNewPromise = function(i, perf, optAsyncResolve) {
  setTimeout(function(){
    perf.log('Creating promise:' + i);
    promise = app.promise(Prom, optAsyncResolve);
